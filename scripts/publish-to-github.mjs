...
В релизе **${tag}** добавлена революционная интеграция с официальным CDN Facepunch для мгновенной загрузки кастомных карт, обновлен маркетплейс плагинов с разделением по площадкам (uMod, CodeFlix, SkyPlugins) и внедрен глобальный ErrorBoundary.

### ⚡ Что нового в ${tag}:
- 🗺️ **1-Click Загрузка кастомных карт на Facepunch CDN**:
  - Нативная интеграция со скрытым API Facepunch (\`api.facepunch.com/api/public/rust-map-upload/\`).
  - Генерация прямых вечных ссылок \`https://files.facepunch.com/rust/maps/...\` без ограничений Dropbox и сторонних хостингов.
  - Анимированный прогресс-бар загрузки (0–100%) в реальном времени.
  - Автоматическая подстановка URL в \`server.levelurl\` и правильная сцена Unity \`Procedural Map\`.
- 🚀 **Zero-Config автозагрузка при старте сервера**: при выборе локального файла \`.map\` сервер перед запуском сам отправит карту в облако Facepunch.
- 🔌 **Маркетплейсы плагинов (uMod, CodeFlix, SkyPlugins)**:
  - Умная фильтрация совместимости плагинов под фреймворк вашего сервера (Oxide vs Carbon).
  - Пагинация каталога, быстрый поиск и развернутое интерактивное досье каждого плагина.
- 🛡️ **Надежность и защита от сбоев (Zero Black Screen)**:
  - Внедрен глобальный \`ErrorBoundary\`, защищающий окно приложения от падений в черный экран.
  - Строгая типизация и валидация входящих ссылок на карты.
- 🌟 Обновленный неоновый бейдж **${tag}** в шапке панели.
