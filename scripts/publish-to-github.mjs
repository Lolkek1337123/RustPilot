...
async function main() {
  ...
  if (getReleaseRes.ok) {
    ...
    const releaseBody = `## 🛸 RustPilot ${tag} — Официальный релиз Pro Edition (Фикс автообновления)

Критическое исправление системы автоматической установки обновлений и перезапуска приложения!

### ⚡ Что нового в ${tag}:
- 🛠️ **Исправлен установщик In-App обновлений**:
  - Полностью ликвидировано зависание черного окна консоли find.
  - Убран нестабильный пайп \`tasklist | find\`, который блокировал stdin на некоторых конфигурациях Windows.
  - Процесс обновления переведен на надежный линейный сценарий с таймаутами и циклом повторных попыток копирования (до 5 раз).
- 🚀 **Поддержка сетевого протокола Rust 2633**: ядро серверов синхронизировано с актуальным Steam BuildID 25191895.
- 🔒 **Обновленный Carbon Production 2.0.259**: автоматическая загрузка и установка свежих бинарников фреймворка Carbon.
